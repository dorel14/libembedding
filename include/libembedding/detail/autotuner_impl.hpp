...
static void create_benchmark_image(std::vector<unsigned char>& out, int width = 32, int height = 32) {
    ...
    lembed_image_tuning_result_t* result)
{
    ...
    for (int t = 0; t < n_threads; t++) {
        for (int b = 0; b < n_batch; b++) {
            current++;
            int threads = threads_options[t];
            int batch = batch_options[b];

            auto r = bench_image_config(model_name, threads, batch, n_images, warmup, bench_iters);
