    ...
    for (int t : valid_threads) {
        for (int b : batch_vec) {
            for (int k : tokens_vec) {
                ...
                double score = score_config(r, objective);
                double best_score = score_config(best, objective);
    ...
    lembed_reranker_tuning_result_t* result)
{
    ...
    if (valid_tokens.empty()) {
        /* No config satisfies min_tokens - use minimum possible */
    ...
    if (best.latency_ms >= 999999) {
        ...
        return lembed::detail::lembed_reranker_autotune(model_name, mode, objective, result);
    }
    ...
    lembed_reranker_tuning_result_t* result)
{
    ...
    return lembed::detail::lembed_reranker_auto_config(model_name, target_ms, LEMBED_OBJECTIVE_BALANCED, result);
}
...
lembed_status_t lembed_reranker_autotune_constrained(
    ...
    lembed_reranker_tuning_result_t* result)
{
    return lembed::detail::lembed_reranker_autotune_constrained(model_name, mode, objective, min_tokens, max_latency_ms, result);
}
...
lembed_status_t lembed_reranker_autotune_custom(
    ...
    lembed_reranker_tuning_result_t* result)
{
    return lembed::detail::lembed_reranker_autotune_custom(model_name, texts, n_texts, mode, objective, result);
}
...
lembed_status_t lembed_reranker_auto_config(
    ...
    lembed_reranker_tuning_result_t* result)
{
    return lembed::detail::lembed_reranker_auto_config(model_name, target_latency_ms, objective, result);
}
    ...
    for (int t = 0; t < n_top_k; t++) {
        for (int w = 0; w < n_weight; w++) {
            for (int s = 0; s < n_storage; s++) {
                current++;
                int top_k = top_k_options[t];
                float min_weight = min_weight_options[w];
                int storage_format = storage_options[s];

                fprintf(stderr, "  [%d/%d] top_k=%d min_weight=%.2f storage=%d\n",
                        current, total, top_k, min_weight, storage_format);

                auto r = bench_sparse_config(
                    model_name, top_k, min_weight, storage_format,
                    threads, batch_size, texts, warmup, bench_iters);
    ...
    lembed_unified_tuning_result_t* result)
{
    ...
    switch (task) {
        ...
        case LEMBED_TASK_RERANKING: {
            ...
            lembed_status_t s = lembed::detail::lembed_reranker_autotune(model_name, mode, LEMBED_OBJECTIVE_BALANCED, &rerank_result);
            if (s != LEMBED_OK) return s;
    ...
    switch (task) {
        case LEMBED_TASK_RERANKING: {
            ...
            lembed_status_t s = lembed::detail::lembed_reranker_auto_config(
                model_name, target_latency_ms, LEMBED_OBJECTIVE_BALANCED, &rerank_result);
            if (s != LEMBED_OK) return s;
            result->threads = rerank_result.threads;
            result->batch_size = rerank_result.batch_size;
            result->max_tokens = rerank_result.max_tokens;
...
/* =========================================================================
 * Image Auto-Tuner Implementation
 * ========================================================================= */

/* Create synthetic image data for benchmarking */
static std::vector<unsigned char> create_synthetic_image_bytes(int width = 224, int height = 224) {
    /* Create a minimal valid PNG image */
    std::vector<uint8_t> raw_data;
    for (int y = 0; y < height; y++) {
        raw_data.push_back(0x00); /* filter byte */
        for (int x = 0; x < width; x++) {
            raw_data.push_back(128); /* R */
            raw_data.push_back(128); /* G */
            raw_data.push_back(128); /* B */
        }
    }
    ...
    auto create_chunk = [&](const char* type, const uint8_t* data, size_t len) -> std::vector<uint8_t> {
        std::vector<uint8_t> chunk;
        uint32_t len_be = (uint32_t)len;
        chunk.push_back((len_be >> 24) & 0xFF);
        chunk.push_back((len_be >> 16) & 0xFF);
        chunk.push_back((len_be >> 8) & 0xFF);
        chunk.push_back(len_be & 0xFF);
        for (int i = 0; i < 4; i++) chunk.push_back(type[i]);
        for (size_t i = 0; i < len; i++) chunk.push_back(data[i]);
        uint32_t crc = crc32(chunk.data() + 4, len + 4);
    ...
    /* PNG signature */
    png.push_back(0x89); png.push_back(0x50); png.push_back(0x4E); png.push_back(0x47);
    png.push_back(0x0D); png.push_back(0x0A); png.push_back(0x1A); png.push_back(0x0A);
    ...
    auto ihdr_chunk = create_chunk("IHDR", ihdr, 13);
    png.insert(png.end(), ihdr_chunk.begin(), ihdr_chunk.end());

    /* IDAT - compress raw data */
    /* Simple zlib compression would be complex; for benchmarking, use a simpler approach */
    /* Just create a minimal valid image for benchmarking purposes */
    return png;
}

/* Benchmark a single image configuration */
static lembed_image_tuning_result_t bench_image_config(
    const char* model_name,
    int threads,
    int batch_size,
    int n_images,
    ...
    int bench_iters)
{
    ...
    auto img_data = create_synthetic_image_bytes();
    ...
    lembed_image_tuning_result_t* result)
{
    ...
    int n_threads = sizeof(threads_options) / sizeof(int);
    int n_batch = sizeof(batch_options) / sizeof(int);

    lembed_image_tuning_result_t best = {0};
    best.latency_ms = 999999;

    int total = n_threads * n_batch;
    int current = 0;
    ...
    for (int t = 0; t < n_threads; t++) {
        for (int b = 0; b < n_batch; b++) {
            current++;
            fprintf(stderr, "  [%d/%d] threads=%d batch=%d\n",
                    current, total, threads_options[t], batch_options[b]);

            auto r = bench_image_config(
                model_name, threads_options[t], batch_options[b],
                n_images, warmup, bench_iters);
